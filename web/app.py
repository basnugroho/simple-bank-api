...
class Register(Resource):
    def post(self):
        ...
        retJson = {
            "status": 200,
            "msg": "You succefully sign up for the API"
        }
        return jsonify(retJson)
...
class Transfer(Resource):
    def post(self):
        ...
        if cash<=0:
            return jsonify(generateReturnDictionary(304, "youre out of money, pleas add or take a loan"))
        
        if not UserExist(to):
            return jsonify(generateReturnDictionary(301, "reveicer username is invalid"))
